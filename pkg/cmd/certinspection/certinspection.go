...
func (o *CertInspectionOptions) Run() error {
	...
	err := visitor.Visit(func(info *resource.Info, err error) error {
		...
		// Using v1beta1 for compatibility with older clusters
		case *certificatesv1beta1.CertificateSigningRequest:
			inspectCSR(castObj)
		default:
			return fmt.Errorf("unhandled resource: %T (%s/%s)", castObj, info.Namespace, info.Name)
...
func printPKIList(pkiList *certgraphapi.PKIList) {
	...
	for _, caBundles := range pkiList.CertificateAuthorityBundles.Items {
		for _, location := range caBundles.Spec.ConfigMapLocations {
			fmt.Printf("configmap/%s[%s]\n", location.Name, location.Namespace)
		}
		for _, cert := range caBundles.Spec.CertificateMetadata {
			fmt.Printf("%s\n\n", certMetadataDetail(cert, pkiList))
		}
	}
}
